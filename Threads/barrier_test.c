...
int main() {
  ...
  status = ult_barrier_init(&barrier, 3);  // main will also wait at the barrier, with count = 4 we get seg fault 
  if (0 != status) {
  ...
  char buf[27];
  time(&now);
  printf("main() waiting at the barrier at  %s", ctime_r(&now, buf));

  ult_barrier_wait(barrier);

  printf("barrier has been lifted!\n");
